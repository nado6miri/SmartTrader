...
var portfolio = { traderStart : 0, config : { }, last_bidask_info : { timetick : 0, tr_price : 0 }, slots : [], idle : false }; // slot config & info......
var portfolio_info = { };
    ...
    */
    ...
    if (process.argv.length === 3 && process.argv[2] == 'resume')
    {
        ...
        for (key in portfolio_list)
        {
            ...
            // for statics
            if (trading_summary.hasOwnProperty(market) === false) { trading_summary[market] = {}; }
            if (trading_summary[market].hasOwnProperty(marketID) === false) { trading_summary[market][marketID] = {}; }

            if (trading_details.hasOwnProperty(market) === false) { trading_details[market] = {}; }
            if (trading_details[market].hasOwnProperty(marketID) === false) { trading_details[market][marketID] = {}; }

            if (netincome_summary.hasOwnProperty(market) === false) { netincome_summary[market] = {}; }
            if (netincome_summary[market].hasOwnProperty(marketID) === false) { netincome_summary[market][marketID] = {}; }

            if (netincome_details.hasOwnProperty(market) === false) { netincome_details[market] = {}; }
            if (netincome_details[market].hasOwnProperty(marketID) === false) { netincome_details[market][marketID] = {}; }
        }
    }
    else // 3. resume mode가 아니면 처음 신규로 DB 초기화 하고 Trade를 시작한다.
    {
        ...
        let traderStart = new Date().toJSON();
        for (key in portfolio_list)
        {
            let market, marketID;
            market = marketID = key;
            ...
            pfolio['slots'] = [];
            pfolio['traderStart'] = traderStart;
...
async function create_new_bid_slot(market, marketID, current, priceinfo)
{
    ...
    if (slots.length === 0)
    {
        ...
        new_bid['amount'] = portfolio_info[market][marketID]['config']['slot_BidAsk_KrwCoin'][0]*1000 / current_price; //priceinfo['trade_price'];
        new_bid['amount'] = 1 * new_bid['amount'].toFixed(6);
        new_bid['invest_KRW'] = portfolio_info[market][marketID]['config']['slot_BidAsk_KrwCoin'][0] * 1000;
    ...
    else // second/others slot creation condition.
    {
        ...
        new_bid['amount'] = portfolio_info[market][marketID]['config']['slot_BidAsk_KrwCoin'][amountIndex]*1000 / current_price; //priceinfo['trade_price'];
        new_bid['amount'] = 1 * new_bid['amount'].toFixed(6);
        new_bid['invest_KRW'] = portfolio_info[market][marketID]['config']['slot_BidAsk_KrwCoin'][amountIndex]*1000;
...
async function add_bid_to_slot(market, marketID, current, priceinfo)
{
    ...
    for (i = 0; i < slots.length; i++)
    {
        ...
        if(bid.length >= config['max_addBidAsk_cnt']) 
        { 
            //console.log("[", market, "][", marketID, "][", i, "] bid.length(count) = ", bid.length, " j = ", j, " exceed max_addBidAsk_cnt[",config['max_addBidAsk_cnt'], "]" );
        }
        else
        {
            if(gap_ratio < (config['new_addBidAsk_Create_Ratio'] + config['new_addBidAsk_Create_Ratio_adj'])) 
            {
                // Order Input (add bid)
                let new_bid = JSON.parse(JSON.stringify(bid_info));
                let orderinfo = { };
...
async function ask_sellCoin_buyKRW(market, marketID, current, priceinfo)
{
    ...
    for(i = 0; i < slots.length; i++)
    {
        let target = config['target_BidAsk_Ratio'] + config['target_BidAsk_Ratio_adj'];
...
async function create_new_ask_slot(market, marketID, current, priceinfo)
{
    ...
    {
        new_slot['type'] = "first";
        new_ask['amount'] = portfolio_info[market][marketID]['config']['slot_BidAsk_KrwCoin'][0]; 
    ...
    else // second/others slot creation condition.
    {
        ...
        if (amountIndex >= 10) { amountIndex = 9; }
        new_slot['type'] = "others";
        new_ask['amount'] = portfolio_info[market][marketID]['config']['slot_BidAsk_KrwCoin'][amountIndex]; 
        new_ask['amount'] = new_ask['amount'] * 1;  
        new_ask['amount'] = 1 * new_ask['amount'].toFixed(6);
...
async function add_ask_to_slot(market, marketID, current, priceinfo)
{
    ...
    for (i = 0; i < slots.length; i++)
    {
        ...
        if(ask.length >= config['max_addBidAsk_cnt']) 
        { 
            //console.log("[", market, "][", marketID, "][", i, "] ask.length(count) = ", ask.length, " j = ", j, " exceed max_addBidAsk_cnt[", config['max_addBidAsk_cnt'], "]" );
            return;
        }
        else
        {
            if(gap_ratio > (config['new_addBidAsk_Create_Ratio'] + config['new_addBidAsk_Create_Ratio_adj']))  // gap_ratio가 정해진 가격보다 상승폭이 크면 coin을 매도하여 KRW를 확보한다.
            {
                // Order Input (add ask)
                let orderinfo = {};
                let new_ask = JSON.parse(JSON.stringify(ask_info));
...
async function bid_sellKRW_buyCoin(market, marketID, current, priceinfo)
{
    ...
    for(i = 0; i < slots.length; i++)
    {
        ...
        let target = config['target_BidAsk_Ratio'] + config['target_BidAsk_Ratio_adj'];  // Coin을 다시 사기위한 현재가격 대비 하락폭 설정
        let cur_eval_net_ratio = cur_slot_statics['cur_eval_net_ratio'];
            ...
            for (marketID in price_infoDB[market])
            {
                ...
                if (mode === 'normal')
                {
                    console.log("[N][", market, "][", marketID, "][", config_filename[market][marketID], "], [Period] = ", config['check_period'], ", [Control Status] = ", config['control_mode'], ", [Algorithm] = ", config['restart_flag'],
                        "[Limit KRW] = ", config['limit_invest_KRW'], ", [Slot별 투입금액(천원)] = ", config['slot_BidAsk_KrwCoin']);
                    //console.log("[N][", market, "][", marketID, "][Control Status] = ", config['control_mode'], ", [real_test_mode] = ", config['real_test_mode']);
                    //console.log("[N][", market, "][", marketID, "][Max 투입제한 금액] = ", config['limit_invest_KRW'], ", [Slot별 투입금액(천원)] = ", config['slot_BidAsk_KrwCoin']);
                    console.log("[N][", market, "][", marketID, "]Idle = ", portfolio_info[market][marketID]['idle'], ", [max_slot_cnt] = ", config['max_slot_cnt'], ", [max_addBidAsk_cnt] = ", config['max_addBidAsk_cnt'], ", [익절 Rate] = ", config['target_BidAsk_Ratio'] + config['target_BidAsk_Ratio_adj'], "%",
                        ", [신규 Slot 생성 Rate] = ", config['new_slot_Create_Ratio'] + config['new_slot_Create_Ratio_adj'], " %, [Add Bid(물타기) Rate] = ", config['new_addBidAsk_Create_Ratio'] + config['new_addBidAsk_Create_Ratio_adj'], "%");
                    //console.log("[N][", market, "][", marketID, "][익절 Rate] = ", config['target_BidAsk_Ratio'] + config['target_BidAsk_Ratio_adj']);
                    //console.log("[N][", market, "][", marketID, "][신규 Slot 생성 Rate] = ", config['new_slot_Create_Ratio'] + config['new_slot_Create_Ratio_adj'], "%, [Add Bid(물타기) Rate] = ", config['new_addBidAsk_Create_Ratio'] + config['new_addBidAsk_Create_Ratio_adj'], "%");
                    //console.log("[N][", market, "][", marketID, "][Add Bid(물타기) Rate] = ", config['new_addBidAsk_Create_Ratio'] + config['new_addBidAsk_Create_Ratio_adj']);
                    //console.log("=====================================================================================================================================");
                ...
                else
                {
                    console.log("[R][", market, "][", marketID, "][", config_filename[market][marketID], "], [Period] = ", config['check_period'], ", [Control Status] = ", config['control_mode'], ", [Algorithm] = ", config['restart_flag'],
                        "[Limit Coin] = ", config['limit_invest_coin'], ", [Slot별 매도 Coin 개수] = ", config['slot_BidAsk_KrwCoin']);
                    //console.log("[R][", market, "][", marketID, "][Control Status] = ", config['control_mode'], ", [real_test_mode] = ", config['real_test_mode']);
                    //console.log("[R][", market, "][", marketID, "][Max 매도제한 Coin 개수] = ", config['limit_invest_coin'], ", [Slot별 매도 Coin 개수] = ", config['slot_BidAsk_KrwCoin']);
                    console.log("[R][", market, "][", marketID, "]Idle = ", portfolio_info[market][marketID]['idle'], ", [max_slot_cnt] = ", config['max_slot_cnt'], ", [max_addBidAsk_cnt] = ", config['max_addBidAsk_cnt'], ", [Coin 재매수 Rate] = ", config['target_BidAsk_Ratio'] + config['target_BidAsk_Ratio_adj'], "%",
                        ", [신규 Slot 생성 Rate] = ", config['new_slot_Create_Ratio'] + config['new_slot_Create_Ratio_adj'], " %, [Add Ask(Coin 고점팔기) Rate] = ", config['new_addBidAsk_Create_Ratio'] + config['new_addBidAsk_Create_Ratio_adj'], "%");
                    //console.log("[R][", market, "][", marketID, "][Coin 재매수 Rate] = ", config['target_BidAsk_Ratio'] + config['target_BidAsk_Ratio_adj']);
                    //console.log("[R][", market, "][", marketID, "][신규 Slot 생성 Rate] = ", config['new_slot_Create_Ratio'] + config['new_slot_Create_Ratio_adj'], "%, [Add Ask(Coin 고점팔기) Rate] = ", config['new_addBidAsk_Create_Ratio'] + config['new_addBidAsk_Create_Ratio_adj'], "%");
                    //console.log("[R][", market, "][", marketID, "][Add Ask(Coin 고점팔기) Rate] = ", config['new_addBidAsk_Create_Ratio'] + config['new_addBidAsk_Create_Ratio_adj']);
                    //console.log("=====================================================================================================================================");
...
async function make_TradingInfomation(current, price_infoDB)
{
    // trade statics
    for (market in price_infoDB)
    {
        for (marketID in price_infoDB[market])
        {
            ...
            trading_details[market][marketID]['list'] = [];
            trading_details[market][marketID]['cur_price'] = cur_price; // update latest current price info.
           ...
           for (let i = 0; i < slots.length; i++)
           {
               ...
               else
               {
                ...
                // tr detail info
                let trinfo = JSON.parse(JSON.stringify(tr_detail_info));
                trinfo['mode'] = mode;
                ...
                if (mode === 'normal')
                {
                    trinfo['normal']['Coin_BidAsk_amount'] = statics['sum_amount_done'];
                    trinfo['normal']['Coin_BidAsk_Average'] = statics['average_withfee'].toFixed(2)*1;
                    trinfo['normal']['BidAsk_KRW'] = statics['sum_invest_KRW_withfee'].toFixed(2) * 1;
                    trinfo['normal']['Coin_KRW_eval'] = statics['cur_eval_net_KRW'].toFixed(2) * 1;
                }
                else
                {
                    trinfo['reverse']['Coin_BidAsk_amount'] = statics['sum_amount_done'];
                    trinfo['reverse']['Coin_BidAsk_Average'] = statics['average_withfee'].toFixed(2)*1;
                    trinfo['reverse']['BidAsk_KRW'] = statics['sum_ask_KRW_withfee'].toFixed(2) * 1;
                    trinfo['reverse']['Coin_KRW_eval'] = statics['cur_eval_net_Coin'].toFixed(2) * 1;
                }
                trading_details[market][marketID]['list'].push(trinfo);
           }

           if (mode === 'normal')
           {
               net_ratio = (sum_net_KRW / (sum_org_KRW + 0.000001)) * 100;
               net_ratio = net_ratio.toFixed(2) * 1;
               average = sum_org_KRW / (sum_org_coin + 0.000001);
               average = average.toFixed(1) * 1;
               sum_org_coin = sum_org_coin.toFixed(2) * 1;
           ...
           {
               net_ratio = (sum_net_coin / (sum_org_coin + 0.000001)) * 100;
               net_ratio = net_ratio.toFixed(2) * 1;
               average = sum_org_KRW / (sum_org_coin + 0.000001);
               average = average.toFixed(2) * 1;
            }
           ...
           // tr summary
           trading_summary[market][marketID] = JSON.parse(JSON.stringify(tr_summary));
           trading_summary[market][marketID]['mode'] = mode;
           trading_summary[market][marketID]['CrDate'] = portfolio_info[market][marketID]['traderStart'];

           let liquidDB = liquidation_DB[market][marketID];
           if(trading_summary[market][marketID]['mode'] == 'reverse') { liquidDB = increasecoin_DB[market][marketID]; }
           trading_summary[market][marketID]['liquidCnt'] = liquidDB.length;
           trading_summary[market][marketID]['SeedSlotCnt'] = slots.length;

           trading_summary[market][marketID]['Net_Ratio'] = net_ratio;

           trading_summary[market][marketID]['Last_Tr_Price'] = last_bidask_price;

           let gap_ratio = (cur_price - last_bidask_price) * 100 / (last_bidask_price + 1); gap_ratio = gap_ratio.toFixed(2) * 1;
           trading_summary[market][marketID]['Gap_Ratio'] = gap_ratio;
           trading_summary[market][marketID]['cur_price'] = cur_price;

           if (mode === 'normal')
           {
                trading_summary[market][marketID]['normal']['Coin_BidAsk_amount'] = sum_org_coin;
                trading_summary[market][marketID]['normal']['Coin_BidAsk_Average'] = average;
                trading_summary[market][marketID]['normal']['BidAsk_KRW'] = sum_org_KRW.toFixed(2)*1;
                trading_summary[market][marketID]['normal']['Coin_KRW_eval'] = sum_net_KRW.toFixed(2)*1;
                //console.log("[N][", market, "][", marketID, "] 투입금액 = ", sum_org_KRW.toFixed(2), ", 매입 Coin = ", sum_org_coin, ", 매입 평단가 = ", average, ", 손익(KRW) = ", sum_net_KRW.toFixed(2)*1, ", 원화 이익율 = ", net_ratio, "%");
            }
           else
           {
                trading_summary[market][marketID]['reverse']['Coin_BidAsk_amount'] = sum_org_coin;
                trading_summary[market][marketID]['reverse']['Coin_BidAsk_Average'] = average;
                trading_summary[market][marketID]['reverse']['BidAsk_KRW'] = sum_org_KRW.toFixed(2)*1;
                trading_summary[market][marketID]['reverse']['Coin_KRW_eval'] = sum_net_coin.toFixed(2)*1;
                //console.log("[R][", market, "][", marketID, "] Coin 매도수량 = ", sum_org_coin.toFixed(2), ", 매도 평단가 = ", average, ", Coin 손익 = ", sum_net_coin.toFixed(2)*1, ", Coin 이익율 = ", net_ratio, "%");
            }
        }
    }

    Save_JSON_latest_file(trading_summary, "./output/trading_summary");
    Save_JSON_latest_file(trading_details, "./output/trading_details");
    make_NetIncomeList(current, price_infoDB);
}
...
var NetTr_detail = 
{ 
    "mode" : 0, "CrDate" : 0, "SlotNo" : 0, "BidAsk_Cnt" : 0, 
...
async function make_NetIncomeList(current, price_infoDB)
{
    for (market in price_infoDB)
    {
        for (marketID in price_infoDB[market])
        {
            netincome_details[market][marketID]['list'] = [];
            netincome_summary[market][marketID] = JSON.parse(JSON.stringify(NetIncome_Summary));
            netincome_summary[market][marketID]['mode'] = portfolio_info[market][marketID]['config']['trade_mode'];

            if(portfolio_info[market][marketID]['config']['trade_mode'] == "normal")
            {
                let sum_org_KRW = 0;
                let sum_net_KRW = 0;
                let liquid_history = liquidation_DB[market][marketID];
                ...
                for (let i = 0; i < liquid_history.length; i++)
                {
                    ...
                    sum_org_KRW += statics['sum_invest_KRW_withfee'];
                    sum_net_KRW += statics['cur_eval_net_KRW'];
                    netincome_summary[market][marketID]['CrDate'] = liquid_history[liquid_history.length - 1]['timetick'];
                    /*
                    console.log("[N][", market, "][", marketID, "][ Slot", i, "] 매도 Coin 가격 =", statics['current_price'],
                        " 매수 평단가 = ", statics['average_withfee'].toFixed(2), ", Coin 잔고 = ", statics['sum_amount_done'].toFixed(2), ", 이익율 = ",
                        statics['cur_eval_net_ratio'].toFixed(2)*100, "%, 이익금액(KRW) = ", statics['cur_eval_net_KRW'].toFixed(2));
                    */
                    let Net_Detail = JSON.parse(JSON.stringify(NetTr_detail));
                    Net_Detail['mode'] = portfolio_info[market][marketID]['config']['trade_mode'];
                    Net_Detail['CrDate'] = liquid_history[i]['timetick'];
                    Net_Detail['SlotNo'] = liquid_history.length - i;
                    Net_Detail['BidAsk_Cnt'] = liquid_history[i]['add_bid'].length;
                    let nettmp = (statics['cur_eval_net_ratio'].toFixed(4) * 1) * 100; nettmp = nettmp.toFixed(2) * 1;
                    Net_Detail['Net_Ratio'] = nettmp;
                    Net_Detail['normal']['Coin_Bid_amount'] = statics['sum_amount_done'].toFixed(2) * 1;
                    Net_Detail['normal']['Coin_Bid_Average'] = statics['average_withfee'].toFixed(2) * 1;
                    Net_Detail['normal']['Bid_KRW'] = statics['sum_bid_KRW_withfee'].toFixed(2) * 1;
                    let reclaimKRW = Net_Detail['normal']['Coin_Bid_amount'] * statics['current_price'];
                ...
                net_ratio = net_ratio.toFixed(2) *1;
                cur_total_KRW = cur_total_KRW.toFixed(2) * 1;
                //console.log("[", market, "][", marketID, "] 총 투자금액(KRW) = ", sum_org_KRW.toFixed(2), ", 현 평가 금액(KRW) = ", cur_total_KRW, ", 이익금액(KRW) = ", sum_net_KRW.toFixed(2), ", 원화 이익율 = ", net_ratio, "%");

                netincome_summary[market][marketID]['normal']['total_invest_KRW'] = sum_org_KRW.toFixed(2) * 1;
                netincome_summary[market][marketID]['normal']['eval_KRW'] = cur_total_KRW;
                netincome_summary[market][marketID]['normal']['net_KRW'] = sum_net_KRW.toFixed(2) * 1;
                netincome_summary[market][marketID]['Net_Ratio'] = net_ratio;
            }
            else
            {
                let sum_org_coin = 0;
                ...
                for (let i = 0; i < increasecoin_history.length; i++)
                {
                    ...
                    sum_org_coin += statics['sum_amount_done'];
                    sum_net_coin += statics['cur_eval_net_Coin'];
                    netincome_summary[market][marketID]['CrDate'] = increasecoin_history[increasecoin_history.length - 1]['timetick'];
                    /*
                    console.log("[R][", market, "][", marketID, "][ Slot", i, "] Coin 매도평단가 = ", statics['average_withfee'].toFixed(2), ", 매도 Coin 수량 = ", 
                    statics['sum_amount_done'], ", 회수금액(KRW) = ", statics['sum_ask_KRW_withfee'].toFixed(2), ", 재매수 가격 = ", statics['current_price'],
                    ", 이익율 = ", statics['cur_eval_net_ratio'].toFixed(2)*100, "%,  Coin 증가개수 = ", statics['cur_eval_net_Coin'].toFixed(2));
                    */
                    let Net_Detail = JSON.parse(JSON.stringify(NetTr_detail));
                    Net_Detail['mode'] = portfolio_info[market][marketID]['config']['trade_mode'];
                    Net_Detail['CrDate'] = increasecoin_history[i]['timetick'];
                    Net_Detail['SlotNo'] = increasecoin_history.length - i;
                    Net_Detail['BidAsk_Cnt'] = increasecoin_history[i]['add_ask'].length;
                    let nettmp = (statics['cur_eval_net_ratio'].toFixed(4) * 1) *100; nettmp = nettmp.toFixed(2) * 1;
                    Net_Detail['Net_Ratio'] = nettmp;
                    Net_Detail['reverse']['Coin_Ask_amount'] = statics['sum_amount_done'].toFixed(2) * 1;
                    Net_Detail['reverse']['Coin_Ask_Average'] = statics['average_withfee'].toFixed(2) * 1;
                    Net_Detail['reverse']['Reclaim_KRW'] = statics['sum_ask_KRW_withfee'].toFixed(2) * 1;
                    let rebidamount = statics['sum_ask_KRW_withfee'] / statics['current_price'];
                ...
                net_ratio = net_ratio.toFixed(2) * 1;
                cur_total_coin = cur_total_coin.toFixed(2) * 1;
                //console.log("[", market, "][", marketID, "] 매도 Coin 합계 = ", sum_org_coin.toFixed(2), ", 재매수 Coin 합계 = ", cur_total_coin, ", Coin 증가 = ", sum_net_coin.toFixed(2), ", 이익율 = ", net_ratio, "%");

                netincome_summary[market][marketID]['reverse']['total_Ask_Coin'] = sum_org_coin.toFixed(2) * 1;
                netincome_summary[market][marketID]['reverse']['reBid_Coin'] = cur_total_coin;
                netincome_summary[market][marketID]['reverse']['net_Coin'] = sum_net_coin.toFixed(2) * 1;
                netincome_summary[market][marketID]['Net_Ratio'] = net_ratio;
            }
        }
    }
    Save_JSON_latest_file(netincome_summary, "./output/netincome_summary");
    Save_JSON_latest_file(netincome_details, "./output/netincome_details");
}
